...
TEST_CASE("invalid construction arguments return specific errors")
{
    ...
    auto zero_temperature = seq::make(
        1, { 1 }, sampling_params { .temperature = 0.0f }, 16);
    ...
    auto nan_temperature = seq::make(
        1,
        { 1 },
        sampling_params { .temperature = std::numeric_limits<float>::quiet_NaN() },
        16);
    ...
    auto infinite_temperature = seq::make(
        1,
        { 1 },
        sampling_params { .temperature = std::numeric_limits<float>::infinity() },
        16);
    REQUIRE_FALSE(infinite_temperature.has_value());
    CHECK(infinite_temperature.error() == seq_errc::invalid_temperature);

    auto zero_output = seq::make(
        1, { 1 }, sampling_params { .max_new_tokens = 0 }, 16);
